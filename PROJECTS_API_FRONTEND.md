...
Этот документ описывает, как фронтенду забирать данные о проектах из Django‑бэкенда и как они соотносятся с текущими блоками на главной (секция «Наши проекты»), страницами портфолио, а также связанными публичными блоками (аккордеон, галерея сервиса, новинки и т.д.).
...
Блок новинок / ограниченных по времени предложений на сайте. Контент задаётся в Django‑админке (модель **Новинка**), публичный список отдаётся отдельным API.

### Модель Novelty (упрощённо)

Поля:

- `id: integer` — уникальный идентификатор.
- `title: string` — название предложения.
- `description: string` — текст описания (может быть пустым).
- `image: string (URL)` — фото; в JSON всегда абсолютный URL.
- `starts_at: string (datetime)` — время запуска предложения (ISO 8601, с часовым поясом).
- `ends_at: string (datetime)` — момент окончания предложения (ISO 8601). Интервал **времени жизни** предложения на клиенте: от `starts_at` до `ends_at`.
- `order: integer` — порядок в списке (0, 1, 2, …); чем меньше число, тем выше в выдаче.

Гарантии со стороны бэкенда для публичного списка:

- В ответ попадают только записи с **`published = true`**.
- Учитывается **текущий момент времени** (на сервере): `starts_at ≤ now ≤ ends_at`. Записи до старта или после окончания в списке **не показываются**.
- Сортировка: `order ASC`, затем более поздний `starts_at`, затем более новый `created_at`.
- `image` — абсолютный URL (как у проектов и аккордеона).

### Эндпоинт списка новинок

**URL:**  
`GET /api/projects/novinki/`

**Пример запроса:**

```http
GET http://127.0.0.1:8000/api/projects/novinki/
Accept: application/json
```

**Ответ:**

Без пагинации — сразу **массив** объектов:

```json
[
  {
    "id": 1,
    "title": "Экспедиция в Арктику: подготовка внедорожника",
    "description": "Узнайте, как мы готовим технику к экспедиции.",
    "image": "http://127.0.0.1:8000/media/novelties/promo.jpg",
    "starts_at": "2026-04-01T09:00:00+03:00",
    "ends_at": "2026-04-30T23:59:59+03:00",
    "order": 0
  }
]
```
...
export type NoveltyListResponse = NoveltyItem[];
```

### Связь с черновым `NewsItem` на фронте

Фронтенд мог опираться на интерфейс вида `NewsItem` (`id`, `title`, `date`, `excerpt`, `content`, `image` — строки). Соответствие полям API новинок:

| Черновой `NewsItem` | API новинок |
|---------------------|-------------|
| `image` | `image` (фото) |
| `title` | `title` (название) |
| `excerpt` / `content` | одно поле `description` (краткий и полный текст можно объединить в админке или позже разделить на бэкенде) |
| `date` | форматировать на клиенте из `starts_at` (например, локализованная дата запуска) |
| — | `ends_at` — для таймера, текста «до …» или скрытия карточки (бэкенд уже отфильтровал истёкшие, но `ends_at` полезен для UI) |
| `id` (строка) | `id` в API — **число**; при необходимости: `String(id)` |

### Использование на фронтенде

- Запрос `GET /api/projects/novinki/` при монтировании блока новинок.
- Пустой массив или ошибка сети — фолбэк (статичные данные, скелетон, скрытие блока), по аналогии с аккордеоном.

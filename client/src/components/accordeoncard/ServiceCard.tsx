...
export const ServiceCard: FC<ServiceCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="group relative flex w-full cursor-pointer items-center overflow-hidden border-b border-black/10 transition-colors"
    >
      <motion.div
        layout
        initial={false}
      ...
      {!isActive && (
        <motion.div 
          className="absolute bottom-0 left-0 h-[2px] bg-[#FF8201] w-0 group-hover:w-full transition-all duration-500"
        />
      )}
    </motion.div>
  )
